...
| `managementLIF` | クラスタ管理LIFまたはSVM管理LIFのIPアドレス。
...
| `svm` | 使用する Storage Virtual Machine

* MetroClusterの場合は省略してください。* <<mcc-best>>。 | SVMの場合に生成されます `managementLIF` を指定します 
...
`useREST` はNVMe/TCPに完全修飾されています。 | `false` 
...
====
スイッチオーバーやスイッチバックの実行中にバックエンド定義を手動で更新する必要がないようにバックエンドを設定できます。 link:../trident-reco/backup.html#svm-replication-and-recovery["SVMレプリケーションとリカバリ"]。
